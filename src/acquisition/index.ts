...
export default class AcquisitionCampaign implements ITwoKeyAcquisitionCampaign {
    ...
    private readonly singletonRegistry: ITwoKeySingletonRegistry;
    ...
    constructor(
        ...
        utils: ITwoKeyUtils,
        erc20: IERC20,
        singletonRegistry: ITwoKeySingletonRegistry,
        sign: ISign
    ) {
    ...
    public create(data: IAcquisitionCampaign, publicMeta:any, privateMeta:any, from: string, {progressCallback, gasPrice, interval, timeout = 60000}: ICreateOpts = {}): Promise<IAcquisitionCampaignMeta> {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                let campaignData = [data.maxReferralRewardPercentWei, data.referrerQuota || 5];

                let {campaignAddress, conversionHandlerAddress, twoKeyAcquisitionLogicHandlerAddress} =
                    await this.singletonRegistry.createProxiesForAcquisitions(
                        addresses,
                        conversionHandlerData,
                        logicHandlerData,
                        campaignData,
                        data.currency,
