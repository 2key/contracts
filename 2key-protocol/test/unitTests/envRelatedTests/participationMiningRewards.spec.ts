...
describe(
    ...
    () => {
        ...
        it('should register participation mining epoch', async () => {
            ...
            // Get latest epoch id
            epochId = parseInt(await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getLatestFinalizedEpochId, []), 10) + 1;
            ...
            await twoKeyProtocol.Utils.getTransactionReceiptMined(
                await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.registerEpoch,
                    [
                        epochId,
                        numberOfUsers,
                        {
            ...
            let epochInProgress = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getEpochIdInProgress,[]);
            expect(parseInt(epochInProgress)).to.be.equal(epochId);
        }).timeout(timeout);

        it('should submit users and their rewards in the epoch', async() => {
            let userRewardsString = userRewards.map((element) => {return element.toString()});

            let txHash = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.assignRewardsInActiveMiningEpoch,
                [
                    epochId,
                    usersInEpoch,
                    userRewardsString,
        ...
        it('should finalize epoch', async() => {
            await twoKeyProtocol.Utils.getTransactionReceiptMined(
                await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.finalizeEpoch,
                    [
                        epochId,
                        {
                            from: twoKeyProtocol.plasmaAddress
            ...
            let isEpochFinalized = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.isEpochRegistrationFinalized,[epochId]);
        ...
        it('should check that after epoch is finalized latest epoch id is the one submitted', async() => {
            let latestEpochId = parseInt(await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getLatestFinalizedEpochId,[]),10);
        ...
        it('should check that total submitted for epoch is equaling sum of all rewards', async() => {
            ...
            let totalRewardsForEpochFromContract = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getTotalRewardsPerEpoch,[epochId]);
        ...
        it('should check that user balances per this epoch are properly set', async() => {
            ...
            for(let i=0; i<usersInEpoch.length; i++) {
                ...
                let userRewardsPerEpochFromContract = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getUserEarningsPerEpoch,[usersInEpoch[i],epochId]);
                // Expect to be same as the submitted value
                expect(parseFloat(userRewardsPerEpochFromContract)).to.be.equal(userRewards[i]);
                let userPendingEpochIds = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getPendingEpochsForUser, [usersInEpoch[i]]);
        ...
        it('should sign user rewards and user address by signatory address', async () => {
            ...
            let pending = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getUserTotalPendingAmount, [user]);
            ...
            let messageSigner = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.recoverSignature,[
                user,
                pending.toString(),
                signature
            ]);
        ...
        it('should submit signature for specific user and check state changes', async() => {
            ...
            let userPendingEpochsBefore = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getPendingEpochsForUser,[user]);
            let userWithdrawnEpochsBefore = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getWithdrawnEpochsForUser,[user]);
            ...
            // Submit signature
            let txHash = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.submitSignatureForUserWithdrawal,[
                user,
                pending,
                signature,
                {
            ...
            let userPendingEpochsAfter = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getPendingEpochsForUser,[user]);
            let userWithdrawnEpochsAfter = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getWithdrawnEpochsForUser,[user]);
            ...
            pending = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getUserTotalPendingAmount,[user]);
        ...
        it('maintainer should set monthly allowance and date from which is counting starting', async() => {
            ...
            let dateStartingCountingMonths = await promisify(twoKeyProtocol.twoKeyParticipationMiningPool.methods.getDateStartingCountingMonths,[]);
            ...
            if(dateStartingCountingMonths.toString() === '0') {
                ...
                await twoKeyProtocol.Utils.getTransactionReceiptMined(
                  await promisify(twoKeyProtocol.twoKeyParticipationMiningPool.methods.setWithdrawalParameters,[
                      blockTimestamp,
                      {
                          from
                      }
                ...
                dateStartingCountingMonths = await promisify(twoKeyProtocol.twoKeyParticipationMiningPool.methods.getDateStartingCountingMonths,[]);
            ...
            let monthlyTransferAllowance = await promisify(
                twoKeyProtocol.twoKeyParticipationMiningPool.methods.getMonthlyTransferAllowance,[]
            );
        ...
        it('should withdraw tokens from mainchain', async() => {
            ...
            let totalAmountOfTokensTransfered = await promisify(twoKeyProtocol.twoKeyParticipationMiningPool.methods.getTotalAmountOfTokensTransfered, []);
            totalAmountOfTokensTransfered = twoKeyProtocol.Utils.fromWei(totalAmountOfTokensTransfered, 'ether').toString();
            ...
            let txHash = await twoKeyProtocol.TwoKeyParticipationMiningPool.withdrawTokensWithSignature(
                signature,
                twoKeyProtocol.Utils.toWei(amountInProgressOfWithdrawal, 'ether').toString(),
                from
            );
        ...
        it('should check if signature is marked as it exists on mainchain', async() => {
            ...
            let isSignatureExistingOnMainchain = await promisify(twoKeyProtocol.twoKeyParticipationMiningPool.methods.isExistingSignature,[signature]);
        ...
        it('should mark that user finished withdrawal on mainchain, and clear his sig, called by maintainer', async() => {
            ...
            let txHash = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.markUserFinishedWithdrawalFromMainchainWithSignature,[
                user,
                signature,
                {
                    from: twoKeyProtocol.plasmaAddress
            ...
            let amountUserWithdrawnUsingSignature = await promisify(twoKeyProtocol.twoKeyPlasmaParticipationRewards.methods.getAmountUserWithdrawnUsingSignature,[
                user,
                signature
            ]);
            ...
            expect(signatureOnContract).to.be.equal(null);
        }).timeout(timeout);
