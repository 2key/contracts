...
const should = require('chai')
    ...
    contract('TwoKeyUpgradableExchange', async (accounts) => {
        ...
        it('Case 5 : Buy Token Positive Test Case', async () => {
            ...
            // let balance = await economyContract.balanceOf(accounts[0]);       /// check balance
        ...
        it('Case 6 : Sell Token Positive Test Case', async () => {
            let investor = '0x90f8bf6a479f320ead074411a4b0e7944ea8c9c1'; // accounts[1];
            let wallet = deployerAddress;   // accounts[2];
            let purchaser = '0xffcf8fdee72ac11b5c542428b35eef5769c409f0'; // accounts[3];
            let adminAcc = deployerAddress; // accounts[4];
        ...
        it('Case 7 : Sell Token negative Test Case Should Give Error if Allowace not Approved', async () => {
            const rate = new BigNumber(2);
            const value = 10000;
        ...
        it('Case 8 : Sell Token negative Test Case Should Give Error if Wallet Holder Does not have token balance', async () => {
            const rate = new BigNumber(2);
            const value = 10000;
            let purchaser = '0xffcf8fdee72ac11b5c542428b35eef5769c409f0'; // accounts[3];
        ...
        it('Case 9 : Sell Token negative Test Case Should Give Error if Only Alive is False', async () => {
            const rate = new BigNumber(2);
            const value = 10000;
        ...
        it('Case 10 : Sell Token negative Test Case Should Give Error Wallet has less Wei than Token Price', async () => {
            const rate = new BigNumber(2);
            const value = 1000;
        ...
        it('Case 11 : Upgrade Exchange Test Case Should set filler with value when Alive', async () => {
              let adminContract = await TwoKeyAdmin.new(deployerAddress);
              let economyContract = await TwoKeyEconomy.new(adminContract.address);
              let exchange = await TwoKeyUpgradableExchange.new(1, deployerAddress, economyContract.address,adminContract.address);
        ...
        // it('Case 13 : Upgrade Exchange Test Case Should give error when not Alive', async () => {
        ...
        it('Case 14 : Upgrade Exchange Test Case Should not give error when called by admin', async () => {
              let adminContract = await TwoKeyAdmin.new(deployerAddress);
              let economyContract = await TwoKeyEconomy.new(adminContract.address);
              let exchange = await TwoKeyUpgradableExchange.new(1, deployerAddress, economyContract.address,adminContract.address);
        ...
        it('Case 15 : Upgrade Exchange Test Case Should give error when called by non admin', async () => {
              let adminContract = await TwoKeyAdmin.new(deployerAddress);
              let economyContract = await TwoKeyEconomy.new(adminContract.address);
              let exchange = await TwoKeyUpgradableExchange.new(1, deployerAddress, economyContract.address,adminContract.address);
        ...
        it('Case 16 : Fallback payable method when alive should add payable amount to exchange contract balance', async () => {
              let adminContract = await TwoKeyAdmin.new(deployerAddress);
              let economyContract = await TwoKeyEconomy.new(adminContract.address);
              let exchange = await TwoKeyUpgradableExchange.new(1, deployerAddress, economyContract.address,adminContract.address);
        ...
        it('Case 17 : Fallback payable method when not alive should add payable amount to new exchange contract', async () => {
              let adminContract = await TwoKeyAdmin.new(deployerAddress);
              let economyContract = await TwoKeyEconomy.new(adminContract.address);
              let exchange = await TwoKeyUpgradableExchange.new(1, deployerAddress, economyContract.address,adminContract.address);
