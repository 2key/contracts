...
contract('DecentralizedNation', async(accounts,deployer) => {
    ...
    it('should return addresses', async() => {
        ...
        console.log("voted yes : " + votedYes.toNumber());

        let votedNo = await weightedVoteContract.voted_no();
        console.log("voted no: " + votedNo.toNumber());

        let weightedyes = await weightedVoteContract.weighted_yes();
        console.log("weighted yes: " + weightedyes.toNumber());
    ...
    it('get vote and choice per address', async() => {
        ...
        let voters = await weightedVoteContract.getAllVoters();
        for(let i=0; i<voters.length; i++) {
            let vote,choice;
            [vote,choice] = await weightedVoteContract.getVoteAndChoicePerAddress(voters[i]);
            console.log(voters[i] + "---" + vote + "---" + choice);
        }
