...
// Initial setup of contracts which are going to be used in tests
    ...
    before(async () => {
        ...
        const campaignStartTime = latestTime() + duration.minutes(5);
        const campaignEndTime = latestTime() + duration.minutes(30);
        const expiryConversion = 123;
        const moderatorFeePercentage = 5;
        const maxReferralRewardPercent = 10;
        const maxConverterBonusPercent = 10;
        const pricePerUnitInEth = 1;
        const minContributionETH = 1;
        const maxContributionETH = 4;
        const conversionQuota = 1;
        /*
